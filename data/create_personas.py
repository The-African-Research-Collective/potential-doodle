...
from litellm_client import get_completion, Generation_Models
...
async def main(args):
    ...
    with jsonlines.open(f"{args.data_directory}/{args.language}_personas.jsonl", "a") as writer, open(file_path, "a") as f:
        ...
        for i, batch in tqdm(enumerate(batch_dataset_generator(dataset['train'], args.batch_size)), total=len(dataset)//args.batch_size):
            ...
            results = await get_completion(batch['templated_prompt'], Generation_Models.GPT4O_MINI, PersonaList)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--language", type=str, default="yo")
    parser.add_argument("--batch_size", type=int, default=10)
    parser.add_argument("--model", type=Generation_Models, choices=list(Generation_Models), default=Generation_Models.GPT4O_MINI)
